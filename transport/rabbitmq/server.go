...
func (s *Server) Start(ctx context.Context) error {
	...
	if s.keepaliveServer == nil {
		s.keepaliveServer = keepalive.NewServer(keepalive.WithServiceKind(KindRabbitMQ))
	}
...
func (s *Server) doRegisterSubscriber(routingKey string, handler broker.Handler, binder broker.Binder, opts ...broker.SubscribeOption) error {
	...
	started = s.started.Load()
	if started {
		old, exists = s.subscribers[routingKey]
	...
	if !started {
		...
		LogWarnf("server stopped, subscription for '%s' deferred to next start", routingKey)
	...
	if exists {
		...
		LogWarnf("subscriber for '%s' already exists, unsubscribing the old one", routingKey)
		if uerr := old.Unsubscribe(false); uerr != nil {
			LogErrorf("unsubscribe old subscriber for '%s' failed: %s", routingKey, uerr.Error())
		}
	}
	return nil
}
