...
func handleHygrothermograph(_ context.Context, topic string, headers broker.Headers, msg *api.Hygrothermograph) error {
	gcpubsub.LogInfof("Topic %s, Headers: %+v, Payload: %+v\n", topic, headers, msg)
	return nil
}
...
// ExampleNewBroker 演示通过 GCP Pub/Sub 驱动订阅主题并按强类型消费消息。
// 没有 Output 注释，go test 只编译不执行；实际运行需要 GCP 项目凭据或本地 Pub/Sub 模拟器 (127.0.0.1:8085)。
func ExampleNewBroker() {
	...
	_, _ = b.Subscribe("test_topic",
		...
		gcpubsub.WithSubscriptionName("test_topic_sub"),
	)
}
