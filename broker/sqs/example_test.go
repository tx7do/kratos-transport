...
func handleHygrothermograph(_ context.Context, topic string, headers broker.Headers, msg *api.Hygrothermograph) error {
	sqs.LogInfof("Topic %s, Headers: %+v, Payload: %+v\n", topic, headers, msg)
	return nil
}
...
// ExampleNewBroker 演示通过 AWS SQS 驱动订阅队列并按强类型消费消息。
// 没有 Output 注释，go test 只编译不执行；实际运行需要本地 ElasticMQ (http://127.0.0.1:9324) 或真实 AWS SQS 凭据。
