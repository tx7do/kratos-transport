...
func handleHygrothermograph(_ context.Context, topic string, headers broker.Headers, msg *api.Hygrothermograph) error {
	stomp.LogInfof("Topic %s, Headers: %+v, Payload: %+v\n", topic, headers, msg)
	return nil
}
...
// ExampleNewBroker 演示通过 STOMP 驱动订阅主题并按强类型消费消息。
// 没有 Output 注释，go test 只编译不执行；实际运行需要本地 STOMP broker (stomp://127.0.0.1:61613，如 ActiveMQ 或 RabbitMQ STOMP 插件)。
func ExampleNewBroker() {
	b := stomp.NewBroker(
		broker.WithCodec("json"),
		broker.WithAddress("stomp://127.0.0.1:61613"),
	)
